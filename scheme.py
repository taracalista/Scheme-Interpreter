...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        procedure = scheme_eval(first, env, True)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        evaluated = scheme_eval(expressions[1], env)
        env.define(target, evaluated)
        return target
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        "*** YOUR CODE HERE ***"
        new_body = Pair(target.second, expressions.second)
        env.define(target.first, (do_lambda_form(new_body, env)))
        return target.first
    else:
...
def do_if_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    value_eval = scheme_eval(expressions.first, env)
    if scheme_true(value_eval):
        return scheme_eval(expressions.second.first, env)
    elif scheme_false(value_eval):
...
def do_and_form(expressions, env):
    ...
    for expression in expressions:
        value_eval = scheme_eval(expression, env)
        if scheme_false(value_eval):
            return False
    return value_eval
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    create_env = env.make_child_frame(nil, nil)
    for clause in bindings:
        check_form(clause, 1, 2)
        if scheme_symbolp(clause.first):
            formals = clause.first
            vals = scheme_eval(clause.second.first, env)
        else:
            raise SchemeError
        create_env.bindings[formals]=vals
    return create_env
